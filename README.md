...
A stratified subject-level train/test split was created on the outcome, then preprocessing applied one-hot encoding to categorical variables, standardization to numeric variables, and encoded ordinal variables. Class imbalance was addressed with SMOTE within the training portion of each cross-validation split, avoiding leakage into the held-out test set. Missing data was either omitted or, optionally, imputed with the median for numeric variables and most-frequent value for categorical/ordinal/binary variables. For each domain, four ML algorithms with grid search over hyperparameters (Logistic Regression, Random Forest, XGBoost, Support Vector Machine) were compared on 5-fold cross-validation F1-score, and the best model per domain was retained for computing held-out test set evaluation.
